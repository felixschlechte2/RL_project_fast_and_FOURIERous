...
agent = SAC(env.observation_space.shape[0]+2, action_space_p1, args)# env.action_space #added +3 for HER
...
models_path = r"./checkpoints/sac_checkpoint_hockey_vs_rand_all_rew_ep_6520_HER"
...
for ep in range(5):
    ...
    for i in range(500): # while not d:
        if i % 100 == 0: print(i)
        env.render()
        action_agent = agent.select_action(np.concatenate([state, [9.16,4]], axis=0), evaluate =True) #here# [:4] # hier # Sample action from policy
