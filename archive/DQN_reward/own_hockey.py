...
class HockeyEnv(gym.Env, EzPickle):
  ...
  def _get_info(self):
    ...
    if self.puck.position[0] < CENTER_X and self.puck.linearVelocity[0] <= 0:
      ...
      max_dist = 250. / SCALE
      max_reward = -50.  # max (negative) reward through this proxy 
      # hier: -50 instead of -30
    ...
    # distance to goal:
    reward_distance_goal = 0
    if self.player1.position[1] > 5:
      max_reward = -1.
      factor = max_reward * (self.player1.position[1]- 5)
      reward_distance_goal = factor
    elif self.player1.position[1] < 3:
      max_reward = -1.
      factor = max_reward * (3 - self.player1.position[1])
      reward_distance_goal = factor
      #print(reward_distance_goal)
    #else: 
    print(self.player1.position[0])
    ...
    return {"winner": self.winner,
            ...
            "reward_distance_goal": float(reward_distance_goal),
            }
...
class HockeyEnv(gym.Env, EzPickle):
  ...
  def _get_info(self):
    ...
    if self.puck.position[0] < CENTER_X and self.puck.linearVelocity[0] <= 0:
      ...
      max_reward = -30.  # max (negative) reward through this proxy
      factor = max_reward / (max_dist * self.max_timesteps / 2)
      reward_closeness_to_puck += dist_to_puck * factor*1.3  # Proxy reward for being close to puck in the own half
    # Proxy reward: touch puck
    reward_touch_puck = 0.
    if self.player1_has_puck == MAX_TIME_KEEP_PUCK:
      reward_touch_puck = 1.
    ...
    if self.player1.position[1] > 5:
      ...
      #print(reward_distance_goal)
    #else: 
    if self.player1.position[0] > 2:
      reward_distance_goal += (max_reward * (self.player1.position[0]-2))/10
