...
def save_statistics(i_episode, episode_reward):
    with open(f"./runs/QR_SAC_{args.env_name}-N_STEPS{args.trajectory_length}-alpha{args.alpha}-quant{args.num_quantile}-bs{args.batch_size}-up_ep{args.updates_per_step}.pkl", 'wb') as f: # ./ReinforcementLearning/ExcercisesGitHub/exercises mit venv/project_code/models/DDPG
...
#Tesnorboard
writer = SummaryWriter('runs/{}_QR_SAC_{}_{}_{}_N_STEPS{}_alpha{}_quant{}_bs{}_up_ep{}'.format(datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"), args.env_name,
                                                             args.policy, "autotune" if args.automatic_entropy_tuning else "", 
                                                             args.trajectory_length, args.alpha, args.num_quantile, args.batch_size, args.updates_per_step))
...
agent.save_checkpoint(args.env_name, args, suffix=f"ep_{0}") # hier
...
for i_episode in itertools.count(1):
    ...
    if i_episode % args.log_test_save[0] == 0: # hier diese Zeile
     ...
     # hier : Intervall wann getestet werden soll
    if i_episode % args.log_test_save[1] == 0 and args.eval is True: 
        testing(env, i_episode, agent, "random", 10, writer, ) # modify: anzahl test episodes and saving interval (see def of function)

    if i_episode % args.log_test_save[2] == 0: # hier 
        agent.save_checkpoint(args.env_name, args, suffix=f"ep_{i_episode}") # hier
        # memory.save_buffer(args.env_name, args, str(i_episode)) # hier
