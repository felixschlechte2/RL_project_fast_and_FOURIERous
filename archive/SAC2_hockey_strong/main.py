...
args.env_name = "hockey_vs_strong_and_self_all_rew"

# hier :
def save_statistics(i_episode, episode_reward):
    with open(f"./runs/SAC_{args.env_name}-stats.pkl", 'wb') as f: # ./ReinforcementLearning/ExcercisesGitHub/exercises mit venv/project_code/models/DDPG
...
for i_episode in itertools.count(1):
    ...
    while not done:
        if args.start_steps > total_numsteps:
            ...
            action_agent = agent.select_action(state)# [:4] # hier # Sample action from policy
            if which_opp(i_episode)[0]: # hier (switching between opponents)
                action_opp = weak_opp.act(obs_agent2)   # np.random.uniform(-1,1,4)
            else:
                action_opp = agent.select_action(obs_agent2) 
        ...
        memory.push(state, action_agent, reward, next_state, mask) # Append transition to memory # hier : ursprünglich action anstelle von action_agent
    ...
    if i_episode % 10 == 0: # hier diese Zeile
        print("Episode: {}, opp: {}, total numsteps: {}, episode steps: {}, reward: {}".format(i_episode, which_opp(i_episode)[1], total_numsteps, episode_steps, round(episode_reward, 2)))
        log("Episode: {}, opp: {}, total numsteps: {}, episode steps: {}, reward: {}".format(i_episode, which_opp(i_episode)[1], total_numsteps, episode_steps, round(episode_reward, 2)))

    test_interval = 50 # hier : Intervall wann getestet werden soll
    if i_episode % test_interval == 0 and args.eval is True: 
        ...
        for _  in range(episodes):
            ...
            while not done:
                action_agent = agent.select_action(state, evaluate=True)# [:4] # hier # Sample action from policy
                action_opp = weak_opp.act(obs_agent2)   # np.random.uniform(-1,1,4)
                action = np.hstack((action_agent, action_opp))
