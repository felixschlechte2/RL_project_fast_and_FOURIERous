...
# Agent 
agent = SAC(env.observation_space.shape[0]+2, action_space_p1, args)
# Opponents 
self_play = SAC(env.observation_space.shape[0]+2, action_space_p1, args)
...
for i_episode in itertools.count(1):
    ...
    while not done:
        ...
        if args.start_steps > total_numsteps:
            ...
            action_agent = agent.select_action(np.concatenate([state, np.array(episode_dict["desired_goal"])], axis=0), evaluate =True)
        ...
        next_state, reward, done, _, info = env.step(action) # Step 
        if opponent != None:
            obs_opp = env.obs_agent_two() 
    ...
    if i_episode % test_interval == 0 and args.eval is True:
        ...
        if i_episode % 10000 == 0:
            memory.save_buffer(f"{i_episode}_HER") 
        avg_reward = 0.
        episodes = 10
        ...
        if i_episode % 200 == 0:
            ...
            agent.save_checkpoint(suffix=f"ep_{i_episode}") 
