...
parser.add_argument('--automatic_entropy_tuning', type=bool, default=True, metavar='G', # HERE changed to automatic entropy adjustment
...
Elos = [1000,1500,1800,1700,1900,2000,2100,1500,1500]
...
for i_episode in itertools.count(1):
    ...
    if ELO_activation:
        Elos[8], Elos[current_opp] = Elo(Elos[8],Elos[current_opp], (info["winner"]+1)/2) 
        Elos[7] = decay_elos([Elos[7]])[0]
    ...
    if i_episode % test_interval == 0 and args.eval is True:
        ...
        print("----------------------------------------")
        print("Test Episodes: {}, Opponent: {}({}), Avg. Reward: {}, win_percentage: {}, score: {}, Elo: {}".format(episodes, opp_string,round(Elos[current_opp],2), round(avg_reward, 2),score[2]/sum(score), score, round(Elos[8],2)))
        print("----------------------------------------")
        log("----------------------------------------")
        log("Test Episodes: {}, Opponent: {}({}), Avg. Reward: {}, win_percentage: {}, score: {}, Elo: {}".format(episodes, opp_string,round(Elos[current_opp],2), round(avg_reward, 2),score[2]/sum(score), score,round(Elos[8],2)))
        log("----------------------------------------")
        
        # activate ELO if test episode condition is satisfied
        if (score[2]/sum(score) >= 0.9 or score[0] == 0) and current_opp >= 2 and current_opp < 7 and not ELO_activation:
            ELO_activation = True
            log("****************************************")
            log(f"NOW THE ELO-SYSTEM IS STARTING. CURRENT ELO: {Elos[8]}")
            log("****************************************")
