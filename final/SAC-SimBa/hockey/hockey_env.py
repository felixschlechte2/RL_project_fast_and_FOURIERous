...
from Box2D.b2 import (edgeShape, circleShape, fixtureDef, polygonShape, revoluteJointDef, contactListener) # type: ignore

import gymnasium as gym # type: ignore
from gymnasium import spaces # type: ignore
from gymnasium.error import DependencyNotInstalled # type: ignore
from gymnasium.utils import seeding, EzPickle # type: ignore
...
class HockeyEnv(gym.Env, EzPickle):
  ...
  def _compute_reward(self):
    ...
    if self.done:
      if self.winner == 0:  # tie
        r += -1 #here instead of 0
      elif self.winner == 1:  # you won
        r += 1 #here 1 instead of 10
      else:  # opponent won
        r -= 1 #here -1 instead of -10
    return float(r)
  ...
  def get_reward(self, info):
    ...
    r += info["reward_touch_puck"]  # hier
    r += info["reward_puck_direction"]  # hier 
    return float(r)
