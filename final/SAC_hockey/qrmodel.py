...
class QNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, num_quantiles, hidden_dim):
    ...
    def q_values(self, state, action):
        q1, q2 = self.quantiles(state, action)
        return torch.mean(q1), torch.mean(q2)
...
class GaussianPolicy(nn.Module):
    ...
    def forward(self, state):
        x = F.relu(self.layer_norm1(self.linear1(state))) #  hier vorher : x = F.relu(self.linear1(state))
        x = F.relu(self.linear2(x))
