...
args.env_name = "hockey_vs_self_all_rew"
...
# hier bis writer :
model_path = r"./old_models/sac_strong"
...
for i_episode in itertools.count(1):
    ...
    episode_stats.save_data(["episode", "reward", "opponent"], [i_episode, episode_reward, 'self'])
    ...
    if i_episode % args.log_test_save[0] == 0: # hier diese Zeile
        print("Episode: {}, opp: {}, total numsteps: {}, episode steps: {}, reward: {}".format(i_episode, 'self', total_numsteps, episode_steps, round(episode_reward, 2)))
        log("Episode: {}, opp: {}, total numsteps: {}, episode steps: {}, reward: {}".format(i_episode, 'self', total_numsteps, episode_steps, round(episode_reward, 2)))
    ...
    if i_episode % test_interval == 0 and args.eval is True: 
        ...
        for _  in range(episodes):
            ...
            while not done:
                ...
                action_opp = opp.act(obs_agent2)   # np.random.uniform(-1,1,4)
                action = np.hstack((action_agent, action_opp))
