...
def sac(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    def update(data):
        ...
        data = {key: torch.tensor(value).to(device) for key, value in data.items()}
    ...
    for t in range(total_steps):
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            print(f"epoch: {epoch}")
            log(str(f"epoch: {epoch}"))

            save_freq = 5 # hier
            # Save model
            if (epoch % save_freq == 0) or (epoch == epochs):
                torch.save(ac.state_dict(), f'./results/SAC_pendulum_{epoch}.pth')
...
if __name__ == '__main__':
    ...
    # env = 'HalfCheetah-v4'
    # , render_mode="human"
    sac(lambda : gym.make(env), actor_critic=core.MLPActorCritic,
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l), 
        gamma=args.gamma, seed=args.seed, epochs=args.epochs,
        logger_kwargs=d)
